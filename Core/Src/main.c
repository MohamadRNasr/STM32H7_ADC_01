...
int main(void)
{
	...
	FRESULT res;
	uint32_t bytesWrite;
	uint8_t wtext[_MAX_SS] = "Hello World \n";
	...
	{
		res = f_mkfs((TCHAR const *)SDPath, FM_ANY, 0, wtext, sizeof(wtext));
		if(res != FR_OK)
		{
			Error_Handler();
		}
		...
		{
			res = f_open(&SDFile, "Temp.txt", FA_CREATE_ALWAYS | FA_WRITE);
			if(res != FR_OK)
			{
				Error_Handler();
			}
	...
	while (1)
	{
		...
		Temp = (110.0 - 30.0) / (TS_CAL2 - TS_CAL1) * (TS_DATA -TS_CAL1) + 30.0;

		sprintf((char *)wtext, "%i - Temperature = %f \r\n", counter, Temp);
		res = f_write(&SDFile, wtext, strlen((char*)wtext), (void *)&bytesWrite);
